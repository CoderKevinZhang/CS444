...
static int sstf_dispatch(struct request_queue *q, int force)
{
	...
	if (!list_empty(&nd -> queue)) 
	{
        ...
        if (sst == begin) 
		{
            ...
            printk(KERN_INFO "[1]-%d-(%llu)", nd -> cur_direction, (unsigned long long)blk_rq_pos(sst));
        ...
        list_for_each(tmp_list_head, &nd -> queue) {
            ...
            if (sstf_distance(ittr_sect, nd -> cur_position) < sstf_distance(sst_sect, nd -> cur_position)) 
			{
				...
				else if ((nd -> cur_direction == BACKWARD) && (ittr_sect < nd -> cur_position))
				{
                    sst = ittr;
                    sst_sect = ittr_sect;
	...
	printk(KERN_INFO "Nikhil's SSTF I/O Scheduler \n");
    return 0;
}
